create database if not exists Ejercicios2;
...
insert into empleados values ('101','Antonio Viguer',45,12,'representante',str_to_date(replace('20/10/86','/','.'),get_format(date,'EUR')),'104',300000,305000),
                            ...
                            ('104','José González',33,12,'dir ventas',str_to_date(replace('19/05/87','/','.'),get_format(date,'EUR')),'106',200000,143000),
                            ...
                            ('107','Jorge Gutiérrez',49,22,'representante',str_to_date(replace('14/11/88','/','.'),get_format(date,'EUR')),'108',300000,186000),
                            ('108','Ana Bustamante',62,21,'dir ventas',str_to_date(replace('12/10/89','/','.'),get_format(date,'EUR')),'106',350000,361000),
                            ('109','María Sunta',31,11,'representante',str_to_date(replace('12/10/99','/','.'),get_format(date,'EUR')),'106',300000,392000),
...
insert into clientes values ('2101','Luis García Antón','106',65000),
							('2102','Álvaro Rodríguez','101',65000),
                            ...
                            ('2111','Critóbal García','103',50000),
                            ('2112','María Silva','108',50000),
							('2113','Luisa Maron','104',20000),
                            ('2114','Cristina Bulini','102',20000),
                            ('2115','Vicente Martínez','101',20000),
							('2117','Carlos Tena','106',35000),
                            ('2118','Junípero Álvarez','108',60000),
                            ('2119','Salomón Bueno','109',25000),
							('2120','Juan Malo','102',50000),
                            ('2121','Vicente Ríos','103',45000),
                            ('2122','José Marchante','105',30000),
							('2123','José Libros','102',40000),
...
select idfab,idproducto,descripcion,precio, (precio * 1.16) as iva_incluido from productos;
select numpedido,fab,producto,cant,importe/cant, importe from pedidos;
select nombre, datediff(curdate(),contrato) as dias_trabajados, 2020 - edad as año_nacimiento from ejercicios2.empleados;
...
select numpedido,fab,producto,cant,importe/cant from pedidos order by importe limit 5;
...
select nombre from empleados where nomre like 'Juan%';

select avg(cuota), avg (ventas) from empleados;
select avg (numpedido), sum(importe), avg(importe/cant) from pedidos;
select round(avg(importe),2) from pedidos where fab = 'ACI';
/*select round(sum(ventas),2) from empleados where nombre like 'Vicente Pantalla';*/
select sum(importe);
select min(fechapedido) from pedidos;
select importe, count(*) from pedidos where importe>25000 group by numpedido;
select oficina, count(*) from empleados group by oficina;
select nombre,numemp, sum(importe) as importe_vendido, clie from empleados inner join pedidos on pedidos.rep = empleados.numemp group by numemp,nombre,clie;
select rep, sum(importe)>30000 as pedidos, avg(importe) as importe_medio from pedidos;
select descripcion, precio, sum(cant) as total from productos inner join pedidos on idproducto=producto group by producto, existencias having sum(cant)>(0.75*existencias) order by 3;
select distinct(oficina) from empleados where ventas> cuota;
select idproducto, descripcion, precio, existencias from productos inner join pedidos on pedidos.cant where having sum(importe)>(0.75*existencias) order by 3;


1.1;
select oficina, dir, region from ejercicios2.oficinas as ofi where region = 'este' or dir is null;
select oficinas.oficina, dir from ejercicios2.oficinas left join empleados on oficinas.oficina = empleados.oficina;
select oficina;


create or replace view ejercicios2_cliente_view as select nombre, repclie from ejercicios2.clientes order by nombre;

select * from ejercicios2.ejercicios2_cliente_view;
create or replace view eejercicios2.jercicios2_empleados_insert_view as insert into ejercicios2.empleados 
values ('111','pancracia', 26, 29, 'Ingeneer', '1999-02-02', '103', 1234.44, 234.647);